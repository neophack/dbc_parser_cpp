...
TEST_CASE("Testing dbc file loading error issues", "[fileio][error]") {
	...
	// The header parser only inspects the very first line for the VERSION tag, so
	// these tests lock in that leading blank lines or comments are rejected rather
	// than silently skipped.
	SECTION("A blank line before the VERSION header throws an error", "[error]") {
 ...
 SG_ Sig1 : 0|8@1+ (1,0) [0|0] "" Vector__XXX)";
		...
		REQUIRE_THROWS_AS(parser->parse_file(filename), Libdbc::DbcFileIsMissingVersion);
	}

	SECTION("A comment line before the VERSION header throws an error", "[error]") {
 ...
 SG_ Sig1 : 0|8@1+ (1,0) [0|0] "" Vector__XXX)";
	...
	SECTION("A signal with a bit size of zero throws a parse error", "[error]") {
 ...
 SG_ Sig1 : 0|0@1+ (1,0) [0|0] "" Vector__XXX)";
	...
	SECTION("A signal larger than 64 bits throws a parse error", "[error]") {
		std::string dbc_contents = PRIMITIVE_DBC + R"(BO_ 234 MSG1: 8 Vector__XXX
