...
TEST_CASE("Testing dbc file loading error issues", "[fileio][error]") {
	...
	// The header parser skips leading blank lines and "// ..." comment banners
	// (some exporters, e.g. Vector CANdb++/Mentor Graphics, prepend one) before
	// looking for the VERSION tag, rather than rejecting the file outright.
	SECTION("A blank line before the VERSION header is skipped", "[error]") {
		std::string dbc_contents = "\n" + PRIMITIVE_DBC + R"(BO_ 234 MSG1: 8 Vector__XXX
 SG_ Sig1 : 0|8@1+ (1,0) [0|0] "" Vector__XXX)";
		const auto filename = create_temporary_dbc_with(dbc_contents.c_str());
		REQUIRE_NOTHROW(parser->parse_file(filename));
		REQUIRE(parser->get_messages().size() == 1);
	}

	SECTION("A comment line before the VERSION header is skipped", "[error]") {
		std::string dbc_contents = "// generated by some tool\n" + PRIMITIVE_DBC + R"(BO_ 234 MSG1: 8 Vector__XXX
 SG_ Sig1 : 0|8@1+ (1,0) [0|0] "" Vector__XXX)";
		const auto filename = create_temporary_dbc_with(dbc_contents.c_str());
		REQUIRE_NOTHROW(parser->parse_file(filename));
		REQUIRE(parser->get_messages().size() == 1);
	...
	SECTION("Loading a dbc file that cannot be opened throws an error", "[error]") {
		REQUIRE_THROWS_AS(parser->parse_file("does_not_exist.dbc"), Libdbc::DbcFileCouldNotBeOpened);
		REQUIRE_THROWS_WITH(parser->parse_file("does_not_exist.dbc"), ContainsSubstring("does_not_exist.dbc"));
	}

	// Signals with an unsupported bit size (outside 1-64) are skipped rather than
	// failing the whole file, since some vendor exports use an out-of-range
	// signal as an undissected blob alongside otherwise well-formed messages.
	SECTION("A signal with a bit size of zero is skipped", "[error]") {
		std::string dbc_contents = PRIMITIVE_DBC + R"(BO_ 234 MSG1: 8 Vector__XXX
 SG_ Sig1 : 0|0@1+ (1,0) [0|0] "" Vector__XXX)";
		const auto filename = create_temporary_dbc_with(dbc_contents.c_str());
		REQUIRE_NOTHROW(parser->parse_file(filename));
		REQUIRE(parser->get_messages().at(0).get_signals().empty());
		REQUIRE_FALSE(parser->unused_lines().empty());
	}

	SECTION("A signal larger than 64 bits is skipped", "[error]") {
		std::string dbc_contents = PRIMITIVE_DBC + R"(BO_ 234 MSG1: 8 Vector__XXX
 SG_ Sig1 : 0|65@1+ (1,0) [0|0] "" Vector__XXX)";
		const auto filename = create_temporary_dbc_with(dbc_contents.c_str());
		REQUIRE_NOTHROW(parser->parse_file(filename));
		REQUIRE(parser->get_messages().at(0).get_signals().empty());
		REQUIRE_FALSE(parser->unused_lines().empty());
	...
	SECTION("A message larger than 255 bytes throws a parse error", "[error]") {
		std::string dbc_contents = PRIMITIVE_DBC + R"(BO_ 234 MSG1: 256 Vector__XXX
 SG_ Sig1 : 0|8@1+ (1,0) [0|0] "" Vector__XXX)";
		const auto filename = create_temporary_dbc_with(dbc_contents.c_str());
		REQUIRE_THROWS_AS(parser->parse_file(filename), Libdbc::DbcFileParseError);
	}
...
TEST_CASE("Loading a dbc file from a path containing Chinese characters", "[fileio][unicode]") {
	// "中文路径测试" (Chinese path test), written as \u escapes so the test's
	// behavior doesn't depend on the source file's own encoding.
	const std::string chinese_dir = u8"中文路径测试";

	std::string dbc_contents = PRIMITIVE_DBC + R"(BO_ 234 MSG1: 8 Vector__XXX
 SG_ Sig1 : 0|8@1+ (1,0) [0|0] "" Vector__XXX)";
	const auto filename = create_temporary_dbc_in_dir_with(chinese_dir, dbc_contents.c_str());
...
TEST_CASE("Parsing a complex real world style dbc file", "[dbc][parsing]") {
	...
	// SENSOR_SONARS uses multiplexed signals (mux markers "M"/"m0"/"m1"); all of
	// them parse, with the mux role recorded on each signal.
	const auto* sonars = parser->get_message_by_id(200);
	REQUIRE(sonars != nullptr);
	REQUIRE(sonars->name() == "SENSOR_SONARS");
	REQUIRE(sonars->get_signals().size() == 10);
	REQUIRE(sonars->get_signals().at(0).is_multiplexor == true);
	REQUIRE(sonars->get_signals().at(0).is_multiplexed == false);
	REQUIRE(sonars->get_signals().at(1).name == "SENSOR_SONARS_err_count");
	REQUIRE(sonars->get_signals().at(1).is_multiplexed == false);
	REQUIRE(sonars->get_signals().at(2).is_multiplexed == true);
	REQUIRE(sonars->get_signals().at(2).multiplex_value == 0);
	REQUIRE(sonars->get_signals().at(6).is_multiplexed == true);
	REQUIRE(sonars->get_signals().at(6).multiplex_value == 1);

	// CM_, BA_, BA_DEF_ and friends aren't parsed, so they're reported as unused.
	REQUIRE(parser->unused_lines().size() == 17);
}

TEST_CASE("Multiplexed message decoding selects the active branch", "[dbc][multiplexing]") {
	auto parser = std::unique_ptr<Libdbc::DbcParser>(new Libdbc::DbcParser());
	REQUIRE_NOTHROW(parser->parse_file(COMPLEX_DBC_FILE));
	...
	// mux value in bits 0..3 selects branch: 0 -> filtered, 1 -> unfiltered.
	std::vector<double> values;

	// mux = 0: multiplexor + err_count + the four m0 signals.
	std::vector<uint8_t> mux_zero(8, 0x00);
	REQUIRE(parser->parse_message(200, mux_zero, values) == Libdbc::Message::ParseSignalsStatus::Success);
	REQUIRE(values.size() == 6);

	// mux = 1: multiplexor + err_count + the four m1 signals.
	std::vector<uint8_t> mux_one(8, 0x00);
	mux_one[0] = 0x01;
	REQUIRE(parser->parse_message(200, mux_one, values) == Libdbc::Message::ParseSignalsStatus::Success);
	REQUIRE(values.size() == 6);

	// mux = 2: no branch matches, only multiplexor + err_count are decoded.
	std::vector<uint8_t> mux_two(8, 0x00);
	mux_two[0] = 0x02;
	REQUIRE(parser->parse_message(200, mux_two, values) == Libdbc::Message::ParseSignalsStatus::Success);
